...
// ADMIN COMPONENT
// ==========================================
export function Admin({ user = {}, activeTab = 'home' }) {
              ...
              <div style={styles.formRow}>
                <div style={{ ...styles.fieldGroup, flex: 1 }}>
                  ...
                  >
                    ...
                    {BANGLADESH_DISTRICTS.filter(d => d !== 'Select District').map((dist) => (
...
// USER COMPONENT
// ==========================================
export function User({ activeTab = 'Home', user = {} }) {
  const [buses, setBuses] = useState([]);
  ...
  return (
    <div style={styles.container}>
          ...
          <div style={styles.modalContentLarge} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalTwoColumn}>
              ...
              <div style={styles.rightInfoColumn}>
                ...
                <div style={styles.detailGridStacked}>
                  <div style={styles.detailCard}>
                    <span style={styles.detailLabel}>Selected Seats</span>
                    <span style={{ ...styles.detailValue, color: '#38bdf8' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <div style={{ ...styles.detailCard, flex: 1 }}>
                      ...
                      <span style={styles.detailValue}>{formatTimeWithAmPm(selectedBus.departureTime)}</span>
                    </div>
      ...
      {isCheckoutOpen && selectedBus && (
        <div style={styles.modalOverlay} onClick={() => setIsCheckoutOpen(false)}>
          <div style={{ ...styles.modalContent, maxWidth: '500px' }} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <div>
                <h3 style={{ margin: 0, color: '#f4f4f5' }}>Complete Reservation</h3>
                <span style={{ fontSize: '0.8rem', color: '#a1a1aa' }}>Total to pay: ৳ {totalAmount}</span>
              </div>
            ...
            <form onSubmit={handleConfirmBooking} style={styles.modalForm}>
              ...
              <div style={styles.fieldGroup}>
                <label style={styles.modalLabel}><Phone size={14} color="#a1a1aa" /> Contact Phone</label>
                <input 
                  type="text" 
              ...
              <div style={styles.fieldGroup}>
                <label style={styles.modalLabel}>Select Payment Method</label>
                  ...
                  {['bKash', 'Nagad', 'Rocket', 'Card'].map((method) => (
                    <button
                      ...
                      style={{
                        padding: '10px',
                        borderRadius: '8px',
                        border: paymentMethod === method ? '2px solid #3b82f6' : '1px solid #3f3f46',
                        backgroundColor: paymentMethod === method ? '#1e3a8a' : '#18181b',
                        color: '#ffffff',
                        fontWeight: '600',
                        cursor: 'pointer'
                      }}
              ...
              <div style={styles.fieldGroup}>
                <label style={styles.modalLabel}>
                  {paymentMethod === 'Card' ? 'Card Number' : `${paymentMethod} Account Number`}
                </label>
                <input 
                  type="text" 
      ...
      {bookingSuccess && (
        <div style={styles.modalOverlay} onClick={() => setBookingSuccess(null)}>
          <div style={{ ...styles.modalContent, maxWidth: '480px', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
            <CheckCircle2 size={56} color="#22c55e" style={{ margin: '0 auto 12px auto' }} />
            <h2 style={{ margin: '0 0 6px 0', color: '#f4f4f5' }}>Ticket Booking Confirmed!</h2>
            <p style={{ color: '#a1a1aa', fontSize: '0.85rem', margin: '0 0 20px 0' }}>
              Your seats have been reserved successfully.
            </p>

            <div style={{ backgroundColor: '#18181b', padding: '16px', borderRadius: '12px', border: '1px solid #3f3f46', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '0.85rem' }}>
              <div><span style={{ color: '#a1a1aa' }}>Operator:</span> <strong style={{ color: '#f4f4f5' }}>{bookingSuccess.busName}</strong></div>
              <div><span style={{ color: '#a1a1aa' }}>Seats Reserved:</span> <strong style={{ color: '#38bdf8' }}>{bookingSuccess.seats.join(', ')}</strong></div>
              <div><span style={{ color: '#a1a1aa' }}>Total Fare Paid:</span> <strong style={{ color: '#22c55e' }}>৳ {bookingSuccess.totalPaid}</strong></div>
              <div><span style={{ color: '#a1a1aa' }}>Payment via:</span> <strong style={{ color: '#f4f4f5' }}>{bookingSuccess.paymentMethod}</strong></div>
              <div><span style={{ color: '#a1a1aa' }}>Transaction ID:</span> <strong style={{ color: '#f4f4f5', fontFamily: 'monospace' }}>{bookingSuccess.trxId}</strong></div>
            </div>

            <button 
              style={{ ...styles.submitModalBtn, width: '100%', marginTop: '20px', padding: '10px' }}
...
const styles = {
  ...
  searchBanner: { backgroundColor: '#27272a', padding: '24px', borderRadius: '16px', border: '1px solid #3f3f46', display: 'flex', flexDirection: 'column', gap: '20px' },
  ...
  resetSearchBtn: { padding: '10px 16px', borderRadius: '8px', border: '1px solid #3f3f46', backgroundColor: '#18181b', color: '#a1a1aa', fontWeight: '600', cursor: 'pointer', height: '42px' },
  ...
  refreshBtn: { display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: '#27272a', color: '#a1a1aa', border: '1px solid #3f3f46', padding: '6px 12px', borderRadius: '8px', fontSize: '0.8rem', cursor: 'pointer' },
  ...
  cardImgContainer: { height: '160px', backgroundColor: '#18181b', position: 'relative' },
  cardImg: { width: '100%', height: '100%', objectFit: 'cover' },
  cardImgPlaceholder: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#71717a', gap: '8px', fontSize: '0.8rem' },
  cardBody: { padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px', flex: 1, justifyContent: 'space-between' },
  operatorTag: { fontSize: '0.75rem', color: '#38bdf8', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px' },
  routeHeaderRow: { display: 'flex', flexDirection: 'column', gap: '2px', marginTop: '2px' },
  routeTitle: { margin: 0, fontSize: '1.15rem', fontWeight: '700', color: '#f4f4f5' },
  busTypeBadgeUnder: { fontSize: '0.75rem', color: '#a1a1aa' },
  coachNumber: { fontSize: '0.75rem', color: '#71717a', display: 'block', marginTop: '4px' },
  timeInfoRow: { display: 'flex', gap: '16px', backgroundColor: '#18181b', padding: '10px 12px', borderRadius: '8px', border: '1px solid #3f3f46' },
  timeBlock: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.78rem', color: '#d4d4d8' },
  cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #3f3f46', paddingTop: '12px' },
  fareValue: { fontSize: '1.2rem', fontWeight: '800', color: '#22c55e' },
  viewSeatsBtn: { display: 'inline-flex', alignItems: 'center', gap: '6px', backgroundColor: '#3f3f46', color: '#ffffff', border: 'none', padding: '6px 14px', borderRadius: '8px', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer', marginTop: '4px' },
  contentSection: { display: 'flex', flexDirection: 'column', gap: '20px' },
  aboutHeader: { backgroundColor: '#27272a', padding: '28px', borderRadius: '16px', border: '1px solid #3f3f46', display: 'flex', alignItems: 'center', gap: '20px' },
  aboutBannerBox: { backgroundColor: '#18181b', padding: '18px 20px', borderRadius: '12px', border: '1px solid #3f3f46', display: 'flex', alignItems: 'flex-start', gap: '14px' },
  ...
  aboutCardTitle: { margin: 0, fontSize: '1rem', fontWeight: '700', color: '#f4f4f5' },
  aboutCardText: { margin: 0, fontSize: '0.82rem', color: '#a1a1aa', lineHeight: '1.4' },
  ...
  infoItem: { backgroundColor: '#18181b', padding: '14px', borderRadius: '10px', border: '1px solid #3f3f46', display: 'flex', alignItems: 'center', gap: '12px' },
  infoLabel: { fontSize: '0.75rem', color: '#a1a1aa', display: 'block' },
  infoValue: { margin: 0, fontSize: '0.9rem', color: '#f4f4f5', fontWeight: '600' },
  emptyHistoryBox: { backgroundColor: '#18181b', padding: '30px', borderRadius: '10px', border: '1px solid #3f3f46', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  historyTicketCard: { backgroundColor: '#18181b', padding: '16px', borderRadius: '12px', border: '1px solid #3f3f46', display: 'flex', flexDirection: 'column', gap: '10px' },
  ...
  successTicketBadge: { backgroundColor: 'rgba(34, 197, 94, 0.15)', color: '#22c55e', border: '1px solid rgba(34, 197, 94, 0.3)', padding: '2px 8px', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '700' },
  historyCardGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '8px', fontSize: '0.82rem' },
  ...
  paymentMethodItem: { backgroundColor: '#18181b', padding: '16px', borderRadius: '12px', border: '1px solid #3f3f46', display: 'flex', alignItems: 'flex-start', gap: '12px' }
};